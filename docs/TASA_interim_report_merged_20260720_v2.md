...
**本 v2 修訂版（2026-07-20）相對於初版合併稿的增修**（本檔案名 `TASA_interim_report_merged_20260720_v2.md`）：

- **釐清 Layer 2「統計偵測層」的正確表述**：契約指定之 CUSUM／BOCPD／SSA 三種**具名方法**確未實作，但本計畫已用三項**等效統計方法**（P4 多窗口滑動掃描、3σ MAD 同儕比較、EP 漂移殘差同儕比較）達成同層功能。故不再以「方法皆無程式碼」的空白式描述帶過，改為正面陳述所採用的替代方法。並明確澄清 **XGBoost／LightGBM 屬 Layer 3（AI 偵測層），非 Layer 2 的替代**，杜絕層級混淆（詳見新增之 A.7.9）。
- **補齊文件指向落差**：初版總覽表將 `anomaly_detector.py`（3σ MAD）與 `ep_slope_detector.py` 一併標註「詳見 Part A §A.7.5」，但 §A.7.5 實際僅涵蓋後者；本版新增 A.7.6 專節記錄 `anomaly_detector.py`，並更新總覽表指向。
- **新增統計偵測層綜述（A.7.9）**：逐項判定 P4／3σ MAD／EP 漂移殘差三法是否構成真正的「統計偵測層」，回答「P4 是否可視為統計偵測層」之提問。
- **新增文獻佐證（附錄 D）**：針對契約「專案背景」痛點第 3 點（偵測靈敏度與誤報率難以兼顧），彙整可佐證此權衡為國際公認且已量化之問題的 6 篇文獻。
- **新增全文參數定義對照表（附錄 E）**：將全文散見之 `neg_streak`、`total_drop_km`、`net_da_km`、θ_a、τ*、`max_tle_gap_h` 等所有參數，統整為單一對照表，附符號、定義、預設值與出處檔案。
...
| Layer 2：統計偵測層（CUSUM／BOCPD／SSA） | ⚠️ 具名方法未實作，已以等效統計方法替代 | 契約指定之 CUSUM／BOCPD／SSA 三種**具名方法**尚無程式碼；但已用三項等效統計方法達成同層功能（P4 滑動窗口／3σ MAD 同儕比較／EP 漂移殘差）。**XGBoost／LightGBM 屬 Layer 3，非本層替代**；現有 Lomb-Scargle 用途不同（長軸旋轉週期估算，非變化點偵測） | Part A §A.7.9、Part D |
| Layer 2 替代方案（現有統計方法） | ⚠️ 部分，未整合進儀表板 | `anomaly_detector.py`（3σ MAD 同儕比較，27,131 顆）、`ep_slope_detector.py`（EP 漂移殘差同儕比較，聯集 Recall+30.9%）、P4 多窗口滑動掃描 | Part A §A.7.5（EP）／§A.7.6（3σ MAD）／§A.7.9（綜述） |
...
電推進衛星單次 Δa 常僅 0.05–0.5 km，低於規則法門檻。`ep_slope_detector.py` 改用同高度層同儕比較法：規則偵測器 Recall=27.1%，EP 偵測器單獨 Recall=39.2%，**聯集 Recall 提升至 58.0%**（+30.9pp），顯示兩法互補；代價是無推進衛星 FAR 上升至 10.1%。此偵測器以「同高度層 × 同 B\* 分位」的無推進（NaN）衛星中位漂移率為基準，計算漂移率殘差 `drift_residual = net_rate_obs − expected_rate`，殘差超過 `EP_KEEP_RESIDUAL`（0.02 km/day）判為 EP_KEEP、淨升軌超過 `EP_RAISE_NET_THR`（0.5 km）判為 EP_RAISE——屬**分布基準（distribution-based）統計偵測**，非閾值規則。

### A.7.6 統計異常偵測器（3σ MAD 同儕比較，非合作目標適用）

> 本節為 v2 修訂新增。初版總覽表將 `anomaly_detector.py` 與 EP 偵測器一併標註「詳見 §A.7.5」，但 §A.7.5 僅涵蓋 EP 偵測器；此處補記本偵測器，修正該文件指向落差。

`anomaly_detector.py` 對 DuckDB `tle_table` 內所有 LEO 衛星（依查詢範圍約 27,131 顆量級）計算 54 天觀測窗之 `net_rate`（首尾 SMA 差 ÷ 觀測天數，km/day），依 9 個高度邊界（200/350/450/550/650/800/1000/1500/2100 km）分箱建立**同儕分布**，以中位數與中位數絕對偏差（MAD）作為對離群值 robust 的基準：

$$\text{anomaly\_score} = \frac{\left|\text{net\_rate} - \text{peer\_median}\right|}{\text{peer\_MAD}}$$

anomaly_score > `SIGMA_THR`（3.0）即旗標，並依方向分為 **RAPID_RAISE**（比同高度層同儕更快升軌 → 可能為 EP 機動、部署升軌、星座調整）與 **RAPID_DESCENT**（比同儕更快降軌 → 可能為主動離軌、高阻力、大氣密度局部增強）。

**關鍵價值**：本法**不依賴推進標註**，可涵蓋軍事衛星、`OBJECT-XXX` 匿名目標、未標註商業衛星等「**非合作目標**」——這是規則偵測器與 LightGBM 分類器（兩者均以推進能力代理 Ground Truth）在方法論上無法覆蓋的族群。推進標註在此僅作為**事後驗證層**，而非偵測依據。**限制**：僅使用觀測窗首尾 SMA，會掩蓋中途往返機動；需 ≥5 筆 TLE 方可分析；MAD 下限設 1e-4 km/day 以避免同高度層漂移率一致時分母為零。

### A.7.7 CDM 弱監督嘗試（負向結果誠實揭露）
...
### A.7.9 統計偵測層（Layer 2）綜述：以等效統計方法替代契約指定方法

> 本節為 v2 修訂新增，用以回答兩個關鍵提問：（一）P4 多窗口補充掃描能否視為一種「統計偵測層」？（二）Layer 2 統計偵測層究竟由何種方法替代？

**結論先行**：契約 Layer 2「統計偵測層」指定的三種**具名演算法**（CUSUM／BOCPD／SSA）尚未實作；但本計畫已用**三項等效的統計方法**填補此層功能——(1) **P4 多窗口滑動掃描**、(2) **3σ MAD 同儕比較異常偵測**（`anomaly_detector.py`，§A.7.6）、(3) **EP 漂移殘差同儕比較偵測**（`ep_slope_detector.py`，§A.7.5）。因此本層的正確表述不是「方法皆無程式碼」，而是「**契約指定的三種具名方法未實作，改以上述三項統計方法達成同層目標**」。

**層級澄清（避免混淆）**：XGBoost 與 LightGBM 屬於契約三層架構中的 **Layer 3（AI 偵測層）**，**不是 Layer 2 的替代方案**。本報告 Part B 之 LightGBM 分類器與附錄 C 之 XGBoost 模型均為**監督式機器學習**（需以推進能力代理 Ground Truth 訓練），與統計偵測層（**無需標籤、以分布統計量判定異常**）在方法論上本質不同，不可將其誤列為 Layer 2 的替代。

**三種方法是否構成「統計偵測層」的逐項判定**：

| 方法 | 統計核心 | 是否為真正的統計偵測 | 說明 |
|:---|:---|:---:|:---|
| P4 多窗口補充掃描 | 固定寬度（7 天 × 4）滑動子窗口重掃 | **部分** | 具「滑動窗口分段掃描」精神，與 sliding-window CUSUM 的分段概念相通；但每個子窗口**內部仍以 Layer 1 的 Δa／Δi／ΔRAAN 閾值判定**，未使用窗內統計量（如序列均值／變異數估計），故嚴格說是 Layer 1 的**時間分段延伸**，非獨立統計量偵測 |
| 3σ MAD 同儕比較（`anomaly_detector.py`） | 高度層中位數 ± MAD、robust z 分數 | **是** | `anomaly_score = \|net_rate − peer_median\| / peer_MAD`，>3σ 旗標。屬教科書級 robust 離群偵測，不需推進標籤，可涵蓋非合作目標 |
| EP 漂移殘差同儕比較（`ep_slope_detector.py`） | B\* 分位分箱同儕分布、漂移率殘差 | **是** | 以同高度層 × 同 B\* 分位的無推進衛星中位漂移率為基準，殘差超門檻即判定。屬分布基準（distribution-based）統計偵測 |

**對提問（一）P4 是否可視為統計偵測層的直接回答**：**可以「部分地」視為**——P4 採用固定寬度滑動子窗口重掃，與統計變化點偵測（如 sliding-window CUSUM）共享「分段掃描」的精神；但因窗內判定仍是 Layer 1 閾值規則、未計算窗內統計量，嚴格而言不算完整的統計偵測方法。真正可**獨立歸類為「統計偵測層」**的，是 **3σ MAD 同儕比較**與 **EP 漂移殘差同儕比較**兩項——兩者皆以「同儕分布統計量（中位數／MAD／分位）＋殘差門檻」判定異常，完全符合統計偵測層之定義，且無需標籤。

**建議正式期中報告採用之表述**：將 Layer 2 狀態由「未實作／方法皆無程式碼」修訂為「**契約具名方法（CUSUM／BOCPD／SSA）未實作，已以三項等效統計方法（P4 滑動窗口／3σ MAD 同儕比較／EP 漂移殘差）達成同層功能，尚待整合進儀表板**」；下一階段最優先工作為將既有統計方法接入 `maneuver_app.py`，而非從零重寫 CUSUM／BOCPD／SSA。
...
| Layer 2 統計偵測層 | ⚠️ | 契約具名方法（CUSUM/BOCPD/SSA）未實作，但已有三項等效統計方法（P4 滑動窗口／3σ MAD 同儕比較／EP 漂移殘差，詳見 Part A §A.7.9）尚未接入本儀表板；現有 Lomb-Scargle 用於長軸旋轉週期估算，用途不同 |
...
本期中進度報告整合了 Layer 1（TLE 差分機動偵測，含 P1–P6 與 54 天全量擴充驗證）、Layer 3（LightGBM 機動分類器，含一項標籤洩漏 bug 的發現與修正）與 `maneuver_app.py`（Streamlit 儀表板，含 TLE 資料缺口守門案例）三個技術面向的最新進度，並與既有的全計畫層級期中預擬版報告（2026-06-24）進行交叉核對，修正其中一項認知落差（Streamlit 儀表板實際進度顯著領先原描述）。Layer 2（統計偵測層）之契約**具名方法**（CUSUM/BOCPD/SSA）維持未實作狀態，如實揭露、不予淡化；惟本計畫已以三項**等效統計方法**（P4 多窗口滑動掃描、3σ MAD 同儕比較、EP 漂移殘差同儕比較，詳見 A.7.9）達成同層功能，且明確界定 XGBoost/LightGBM 屬 Layer 3、非本層替代。將既有統計方法整合進 `maneuver_app.py`（而非從零重寫 CUSUM/BOCPD/SSA）建議列為下一階段最優先工作。所有量化指標均附具體檔案路徑或函式名稱作為證據，圖表以真實計算/執行結果為主，示意圖表均已明確標註。
...
## 附錄 D：偵測靈敏度與誤報率權衡之文獻佐證

> 本附錄為 v2 修訂新增，對應契約「專案背景」痛點第 3 點——**偵測靈敏度（Recall）與誤報率（FAR）難以兼顧**。以下彙整國際同行文獻，佐證此權衡是 TLE／雷達機動偵測領域公認且已被量化的核心難題，而非本計畫特有；並說明本計畫 P1–P6、Recall@N、閾值優化（τ\*）等設計正是針對此權衡的工程回應。

**D.1 權衡的量化證據（核心佐證）**

- **Adaptive CuSum（自適應累積和法），Cislunar/LEO 機動偵測** [D1]：以自適應資料驅動門檻的序列統計方法對 6 顆 LEO RSO 進行 TLE 機動偵測，可在事件發生後數個資料點內達 **>96% 命中率**，同時將誤報壓低至**約每 400 天 1 次**；另一組結果對所有 RSO 達 **>95%（最高 98%）** 偵測率、誤報頻率**低至每 200～3000 天 1 次**。該文明確指出：**σ 偵測門檻可調高以提升偵測可靠度，但代價是誤報頻率上升**；且**提高門檻使誤報率呈指數下降，偵測延遲卻呈線性上升**——這正是靈敏度／誤報率權衡的教科書式量化表述，可直接引用以佐證痛點 3。

- **Kelecy 等人，TLE 機動偵測奠基工作（AMOS 2007）** [D2]：系統性研究以相鄰 TLE 對之半長軸差值作為機動指標，指出 TLE 噪音水準直接限制可偵測的最小 Δa，門檻設定必然在漏報與誤報間取捨——本計畫 Part A 的 P2 高度自適應閾值與 P5 太陽通量倍率即為此問題的細化處理。

- **簡化攝動模型 + 軌道根數變化法（MDPI Applied Sciences, 2021）** [D3]：以 SGP4 簡化攝動模型搭配軌道根數變化偵測機動，強調需在去除大氣阻力／J2 等自然攝動後方能設定合理門檻，否則自然漂移將淹沒真實機動信號——與本計畫 J2 修正與 P1 單調衰減抑制策略動機一致。

**D.2 統計偵測層方法之文獻依據（呼應 A.7.9）**

- **Bayesian Online Changepoint Detection（BOCD），均值空間 vs. proper-element 空間** [D4]：將 BOCD 應用於軌道機動偵測並比較兩種元素空間之效能，屬契約 Layer 2 指定之 BOCPD 方法的代表性文獻，可作為後續補實作之參考基準。

- **幾何距離差之即時 LEO 機動偵測（MDPI Aerospace, 2025）** [D5]：以幾何距離差進行即時 LEO 機動偵測，展示同儕／基準比較式方法的可行性，與本計畫 3σ MAD 及 EP 漂移殘差同儕比較法（§A.7.6／§A.7.5）思路相通。

**D.3 評估方法學佐證（呼應 Recall@N 與 ROC）**

- **ROC 曲線於異常偵測之門檻選擇** [D6]：ROC 曲線以偽陽性率為 x 軸、真陽性率為 y 軸，於全操作範圍呈現偽陽性與偽陰性的取捨，是決定「在可接受誤報率下的最適門檻」的標準工具——本計畫 Part B 圖 B-4 的真實 ROC 曲線與 Part A 的 Recall@N／Precision@N 曲線即依此方法學呈現靈敏度／誤報率的完整操作曲線，而非單點指標。

**附錄 D 參考文獻**

- [D1] *Detection of Satellite Maneuvers in Earth and Cislunar Orbits using Adaptive CuSum Methods*, Engineering Archive / ION JNC. https://engrxiv.org/preprint/view/7254 ；https://www.ion.org/jnc/abstracts.cfm?paperID=16648
- [D2] T. Kelecy et al., *Satellite Maneuver Detection Using Two-line Element (TLE) Data*, AMOS Conference, 2007. https://amostech.com/TechnicalPapers/2007/Modeling_Analysis_Simulation/Kelecy.pdf
- [D3] *Simplified Approach to Detect Satellite Maneuvers Using TLE Data and Simplified Perturbation Model Utilizing Orbital Element Variation*, Applied Sciences, 11(21):10181, 2021. https://www.mdpi.com/2076-3417/11/21/10181
- [D4] *RSO proper elements: Concept, methods, and application to maneuver detection*（含 Bayesian online changepoint detection 於 mean- 與 proper-element 空間之比較）, Advances in Space Research. https://www.sciencedirect.com/science/article/abs/pii/S0273117723006865
- [D5] *Real-Time Detection of LEO Satellite Orbit Maneuvers Based on Geometric Distance Difference*, Aerospace, 12(10):925, 2025. https://www.mdpi.com/2226-4310/12/10/925
- [D6] *Comprehensive Analysis of Receiver Operating Characteristic (ROC) Curves for Anomaly Detection*, IEEE. https://ieeexplore.ieee.org/document/9909988/

---

## 附錄 E：全文參數定義對照表

> 本附錄為 v2 修訂新增，將全文散見之所有參數／特徵／門檻統整為單一對照表，供審查者交叉核對。「出處」欄標示該參數之權威定義來源檔案。

**E.1 Layer 1 差分偵測與 P1–P6 策略參數**

| 參數／符號 | 中文定義 | 預設值／門檻 | 出處檔案／章節 |
|:---|:---|:---|:---|
| `Δa` / `da_km` | 相鄰 TLE 對半長軸差值（km） | — | `validate_annotations.py`／A.3.1 |
| `Δi` | 相鄰 TLE 對傾角差值（度） | 機動特徵 \|Δi\|>0.01° | A.3.1–A.3.2 |
| `Δe` | 相鄰 TLE 對離心率差值 | 輔助確認 | A.3.1 |
| `ΔRAAN_res` / `max_draan_res_deg` | J2 校正後 RAAN 殘差（度） | \|殘差\|>0.5° | A.3.1／表 B-1 |
| `θ_a`（theta_a） | 半長軸差分基礎閾值（km） | 基礎 1.0；P2 分層 0.5/1.0/2.0 | A.3.2／A.4.2 |
| `neg_streak` | 連續負 Δa（下降）次數 | P1 門檻 ≥5；高 B\* 時降為 3 | `validate_annotations.py:38,184`／A.4.1 |
| `total_drop`／`total_drop_km` | 累積下降量＝Σ\|負向 Δa\|（km） | P1 門檻 >5 km | `validate_annotations.py:182`／A.4.1 |
| `net_da`／`net_da_km` | 觀測期淨 Δa（末−首，正=升軌）（km） | P1 門檻 <−3 km | `validate_annotations.py:181`／A.4.1 |
| `monotone_decay` | 單調衰減旗標（三條件同時成立） | neg_streak≥s∧total_drop>d∧net_da<n | `validate_annotations.py:203`／A.4.1 |
| `pos_da_spikes` | 正方向 Δa 激增次數（> 均值） | 救援門檻 ≥2 | `validate_annotations.py:209`／A.4.1 |
| `monotone_with_spike` | 單調衰減但含激增（觸發救援） | monotone_decay∧pos_da_spikes≥2 | `validate_annotations.py:210`／A.4.1 |
| `s_thr` / `d_thr` / `n_thr` | P1 三門檻（P3 可放寬之變數） | 預設 5／5km／−3km | `validate_annotations.py:203`／A.4.3 |
| `bstar_mean` | 觀測期平均 B\* 阻力係數 | P3 門檻 >5×10⁻⁴ | A.4.3／表 B-1 |
| `multi_window_detected` | P4 多窗口旗標（任一子窗口觸發） | 4×7 天不重疊子窗口 | `validate_annotations.py:345,419`／A.4.4 |
| F10.7 | 太陽輻射通量（sfu） | P5：>100/150/200→×1.2/1.5/2.0 | A.4.5 |
| P6 前綴閾值 | 星座名稱前綴專屬 θ_a | KUIPER-→0.3；ISS/PROGRESS 等→5.0 km | A.4.6 |

**E.2 Layer 3 LightGBM 20 特徵（`Orbital_Maneuver_V2/dataset.py`）**
...
| `monotone_decay` | 單調衰減旗標 | 8.1% |
| `da_std` | Δa 標準差 | 5.3% |
| `alt_km` | 平均軌道高度（km） | 5.2% |
| `bstar_f107_normalized`（新增） | B\* 對 F10.7 正規化值 | 2.5% |
| `max_tle_gap_h` | 視窗內最大 TLE 間隔（小時） | 1.8% |
| `max_draan_res_deg` | 最大 J2 校正 RAAN 殘差（度） | 1.5% |
| `ecc` | 離心率 | 1.1–1.4% |
| `inc_deg` | 傾角（度） | 1.1–1.4% |
| `dv_net_ms` | 淨速度增量估算（m/s） | 1.1–1.4% |
| `da_abs_mean` | 逐次轉移 \|Δa\| 均值 | 1.1–1.4% |
| `total_drop_km` | 累積下降量（km） | 0.4–0.7% |
| `n_transitions` | TLE 轉移次數 | 0.4–0.7% |
| `net_da_km` | 淨 Δa（km） | 0.4–0.7% |
| `mean_tle_gap_h` | 平均 TLE 間隔（小時） | 0.4–0.7% |
| `n_tle` | TLE 筆數（資料密度） | 0.1% |
| `neg_streak` | 連續負 Δa 次數 | 0.1% |
| `inc_family_enc` | 傾角族群類別編碼 | 0.0% |
| `da_monotonic_decay`（新增） | 純阻力衰減旗標 | 0.0% |

**E.2.1 已剔除之標籤洩漏特徵（tautological predictor）**

| 特徵名稱 | 剔除原因 |
|:---|:---|
| `flag_rate` | 與標籤 `maneuver_detected` 共用同一套規則旗標邏輯 |
| `n_flagged` | 同上，構成 tautological 預測（可致 AUC 虛高至 1.0） |
| `n_windows_flagged` | 同上 |
| `burn_freq_per_day` | 同上 |

**E.3 LightGBM 訓練與閾值參數**

| 參數 | 定義 | 現況值 |
|:---|:---|:---|
| `τ*`（tau） | F₀.₅ 最大化決策閾值 | 0.5747（初版 0.8901） |
| `n_estimators` | 最大樹數（早停） | 1000，實收斂於第 188 棵 |
| `learning_rate` | 學習率 | 0.05 |
| `num_leaves` | 每棵樹葉數 | 15 |
| `min_child_samples` | 葉節點最小樣本 | 10 |
| `reg_lambda` | L2 正則化 | 1.0 |
| `class_weight` | 類別權重 | balanced |
| `early_stopping_rounds` | 早停容忍輪數 | 50 |
| `random_state` | 隨機種子 | 42 |

**E.4 統計偵測層參數（§A.7.5–A.7.6）**

| 參數 | 定義 | 預設值 | 出處檔案 |
|:---|:---|:---|:---|
| `SIGMA_THR` | 異常分數旗標門檻（σ 倍數） | 3.0 | `anomaly_detector.py:44` |
| `peer_median` | 同高度層 net_rate 中位數 | 動態 | `anomaly_detector.py` |
| `peer_MAD` | 中位數絕對偏差（robust σ 估計） | 下限 1e-4 | `anomaly_detector.py:103` |
| `anomaly_score` | \|net_rate−peer_median\|/peer_MAD | — | `anomaly_detector.py:114` |
| `net_rate` | (SMA_last−SMA_first)/obs_days（km/day） | — | `anomaly_detector.py:77` |
| `MIN_TLE` | 最低 TLE 筆數 | 5 | `anomaly_detector.py:45` |
| `EP_RAISE_NET_THR` | EP 淨升軌判定門檻（km） | 0.5 | `ep_slope_detector.py:61` |
| `EP_THRUST_MIN` | EP 推力估算最小值（km） | 1.0 | `ep_slope_detector.py:62` |
| `EP_VAR_MIN` | da_std/da_abs_mean 最小比值 | 0.8 | `ep_slope_detector.py:63` |
| `EP_KEEP_RESIDUAL` | EP 維站漂移殘差門檻（km/day） | 0.02 | `ep_slope_detector.py:74` |
| `ep_thrust_km` | total_drop_km−\|net_da_km\|（累積推力估算） | — | `ep_slope_detector.py` |
| `drift_residual` | net_rate_obs−expected_rate（km/day） | — | `ep_slope_detector.py:244` |
| `TLE_GAP_SUPPRESS_H` | ML 判定之 TLE 缺口壓制門檻（小時） | 48.0 | `maneuver_app.py`／C.4 |
...
| Recall（召回率） | TP/(TP+FN) |
| F₁ / F₀.₅ | 精確率與召回率之調和平均（F₀.₅ 偏重精確率） |
| FAR（誤報率） | FP/(FP+TN)，負例中被誤判為機動之比例 |
| Recall@N | 依偵測信心排名前 N 名內涵蓋的真實機動比例 |
| Precision@N | 排名前 N 名中真實機動之比例 |
| AUC-ROC | ROC 曲線下面積 |
| ΔV | 速度增量（m/s），合成資料生成範圍 [0.001, 50] |
