...
> **圖二**：（a）（b）（c）三張子圖所示之類別分布／切分／基準分類器（naive classifier，全預測多數類別）示意為初版資料（91.96% 無機動）之示意版本，用於說明不平衡場景的一般性方法論問題（為何不能只看 Accuracy）；目前實際訓練資料的正例比例已提升至 20.7%（見上文），示意圖之具體百分比不代表現況，圖表更新列入後續規劃。
...
| `class_weight` | None | **"balanced"** | 自動設定類別權重 $w_i = N/(n_c \cdot N_i)$，用於補償現況約 1:3.8 之正負例比例（1:11.5 僅屬初版資料設定，不作為現況模型依據，見 3.1 節） |
...
2. **特徵世系（feature lineage）僅完成一輪稽核**：已確認並移除四個與標籤直接共用邏輯的特徵，但尚未針對其餘 20 個特徵逐一建立完整的特徵世系表（計算時間範圍、是否可於實際告警時刻取得等），不能完全排除更細微的時間或管線洩漏。
...
未來工作將依上述限制依序展開：（1）以時間外推與 rolling-origin 方式重新驗證泛化能力；（2）完成完整特徵世系稽核；（3）補上 PR-AUC、校準分析與門檻不確定性報告；（4）以多 seed／OOF 方式驗證 SHAP 排名穩定性；（5）在保持與論文一標籤獨立性的前提下，引入 MEME 等獨立標籤來源直接參與訓練，縮小外部驗證揭露的泛化落差；（6）統一 `build_training_dataset.py` 內部不一致的觀測窗口文件說明（26/30/54 天）；（7）擴展至多分類任務（電推進/化學推進/微推力），為機動意圖分析提供更細粒度的輸出。
