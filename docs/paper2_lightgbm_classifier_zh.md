...
> **先看結論**：本節內部測試集結果（5.1–5.3 前段）呈現的是模型對論文一規則標籤的重現程度，Precision/Recall 均在 97% 以上；但這**不代表模型對真實機動事件的判斷力**。5.3 節末段之獨立外部驗證（Plan A，Starlink MEME 精密星曆，非本文標籤來源）顯示 Recall 降至 **39.7%**（Precision 維持 100%）——這是本文對模型真實泛化能力最誠實的量化結果，建議讀者優先參照此數字評估模型的實際可信度，而非僅看內部測試集的高分。
