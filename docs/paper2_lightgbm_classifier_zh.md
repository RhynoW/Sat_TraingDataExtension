...
此標籤策略避免了對機密資料的依賴，但正負例比例嚴重不平衡（約 1:11.5），需在模型訓練中特別處理。
...
| 特徵名稱 | 物理意義 | 類型 |
|:---------|:---------|:----:|
| `alt_km` | 平均軌道高度（km） | 軌道幾何 |
| `inc_deg` | 平均傾角（度） | 軌道幾何 |
| `ecc` | 平均離心率 | 軌道幾何 |
| `inc_family_enc` | 傾角族群編碼（0°/30°/53°/90°等） | 類別 |
| `net_da_km` | 30 天淨半長軸變化（km） | 軌道動力學 |
| `max_da_km` | 最大單步 Δa（km） | 軌道動力學 |
| `da_std` | Δa 標準差（km） | 軌道動力學 |
| `da_abs_mean` | 平均 |Δa|（km） | 軌道動力學 |
| `max_di_deg` | 最大單步 Δi（度） | 軌道動力學 |
| `max_draan_res_deg` | 最大 J2 修正後 ΔRAAN 殘差（度） | 軌道動力學 |
| `neg_streak` | 最長連續負 Δa 次數 | 阻力特徵 |
| `total_drop_km` | 累積軌道高度下降量（km） | 阻力特徵 |
| `monotone_decay` | 單調衰減旗標（0/1） | 阻力特徵 |
| `n_transitions` | Δa 正負號變換次數 | 動力學特徵 |
| `n_flagged` | 觀測期內旗標 TLE 對總數 | 機動頻率 |
| `flag_rate` | 旗標率（n_flagged / 觀測次數） | 機動頻率 |
| `burn_freq_per_day` | 每日估算點火頻率 | 機動頻率 |
| `n_windows_flagged` | 含旗標的子窗口數量 | 多窗口信號 |
| `n_tle` | 觀測期 TLE 總筆數 | 資料密度 |
| `mean_tle_gap_h` | 平均 TLE 間隔（小時） | 資料密度 |
| `max_tle_gap_h` | 最大 TLE 間隔（小時） | 資料密度 |
| `dv_net_ms` | 估算淨速度增量（m/s） | 推算機動強度 |
...
**表 3：四種分類方法在測試集上的效能比較**

| 方法 | Precision | Recall | F₁ | AUC-ROC |
|:-----|----------:|-------:|---:|--------:|
| 規則基準（flag_rate > 5%） | 64.7% | 32.5% | 43.3% | 0.974 |
| 隨機森林（300 棵樹） | 66.4% | 99.4% | 79.6% | 0.988 |
| XGBoost（300 棵樹） | 64.3% | 98.2% | 77.8% | 0.990 |
| **LightGBM（本研究，561 棵樹）** | **81.6%** | **68.0%** | **74.2%** | **0.990** |
...
LightGBM 在 AUC-ROC 與隨機森林/XGBoost 相近（0.990 vs 0.988/0.990）的同時，透過閾值優化策略將精確率從約 65% 提升至 81.6%，體現了 F-beta 閾值優化的實際效果。

### 5.2 SHAP 特徵重要性分析

圖 1 呈現 SHAP TreeExplainer 在測試集上計算的特徵貢獻（以平均絕對 SHAP 值佔全特徵總和的比例表示）。

**表 4：SHAP Top-10 特徵重要性**
...
在測試集的混淆矩陣中（表 5），LightGBM 於閾值 $\tau^* = 0.8901$ 下：

**表 5：LightGBM 測試集混淆矩陣（τ = 0.8901）**
