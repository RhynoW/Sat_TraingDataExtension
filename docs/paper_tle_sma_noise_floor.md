...
表 1 為 **13 顆** IDS 測高衛星於認證安靜期之實測 σ——**全數穩健納入**（各 22–200 個認證安靜區間、數千 TLE 點），無因取樣不足而略去者。

**表 1　IDS 測高衛星 TLE 半長軸雜訊實測（認證安靜期）**

| 衛星 | NORAD | 高度 (km) | 安靜區間數 | 點數 | σ_res (m) | σ_diff (m) | 表 11 假設 (m) | 實測/表 11 |
|---|---|--:|--:|--:|--:|--:|--:|--:|
...
> 註：本表為 **13 顆 IDS 測高衛星之全數實測**（各 22–200 個認證安靜區間、數千 TLE 點，估計穩健），依 σ_diff 由小至大排序。σ_diff 中位 **0.2 m**、>800 km 帶（10 顆）中位亦 0.2 m，較表 11 假設之 50 m 小約兩個數量級。HY-2A（968 km、1.2 m）與 Envisat（766 km、0.5 m）略高，反映較低高度／老舊衛星之較大阻力擾動，仍屬次公尺級。（勘誤：前版誤用 HY-2D=49206、SWOT=55160 二個 OneWeb 編號並因而排除，本版更正為 **HY-2D=48621、SWOT=54754** 之真值，兩者現皆穩健納入。）
...
**圖 1** 以對數縱軸呈現 σ 與高度之關係：≥700 km 之 13 顆測高衛星密集落於 0.1–0.5 m 之次公尺平台（中位 0.2 m），福衛三號（788 km，無推進）約 1 m，而 Starlink 低軌帶（350–550 km）高達 24–75 m，兩者相差約兩個數量級（約 1/100–1/375）；既有門檻表對高軌之 50 m 假設（灰虛線）遠高於實測，凸顯其在高高度帶之高估。

**核心結果**：於涵蓋 **719–1338 km** 之 **13 顆** IDS 測高衛星（全數、各 22–200 個認證安靜區間），σ_diff 中位為 **≈0.2 m**（>800 km 帶 10 顆中位亦 0.2 m），最佳者低至 **0.1 m**（Jason-2/3、Sentinel-6A）。此值：

1. **僅為表 11 假設（50 m）的約 0.4%**——證明既有門檻表在高高度帶高估雜訊達**兩個數量級**；
2. 較低軌 **Starlink 帶（~350–550 km）之 σ≈24–75 m** 小約**兩個數量級（約 1/100–1/375）**（Starlink 之 σ 由本專案逐星 σ 校準得 ~25–50 m，與此比值一致）。
...
1. **門檻應以高度相依 σ 正規化**。σ 於 719–1338 km 為 0.1–0.5 m（中位 0.2 m）、於 350–550 km 為 24–75 m，跨度達兩個數量級。以單一絕對 |Δ*a*| 門檻套全高度帶，必然在高軌過度保守、在低軌淹沒小機動；正解為將門檻表示為 $k\cdot\sigma(h)$，即 SNR 判定。
2. **既有門檻表之高軌帶須修正**。表 11 對 ≥700 km 假設 σ=50 m，實測中位僅 0.2 m（比值 ~0.004），高估兩個數量級；沿用將使高軌機動之靈敏度被人為壓低。
...
**限制**：13 顆均以數十至二百個認證安靜區間、數千點估計，穩健；其中 HY-2A（968 km、σ_diff 1.2 m）與 Envisat（766 km、0.5 m）略高，係較低高度／老舊衛星之阻力擾動較大所致，仍屬次公尺級。福衛三號之較高 σ（約 1 m）反映老化與衰減段之 TLE 品質，屬保守上界。

## 6. 結論

本文以 **13 顆** IDS DORIS 測高衛星於操作方認證安靜期之全數實測，首次在 719–1338 km 帶校準 TLE 半長軸之純雜訊底，得 σ_diff 中位 **≈0.2 m**（最佳 0.1 m），較低軌 Starlink 小約**兩個數量級**，並證明此雜訊底**強烈隨高度變化**、既有門檻表在高軌高估雜訊達兩個數量級。將此高度—σ 關係應用於福衛系列，福衛三號（無推進）確立 ~1 m 在地雜訊底，福衛五號之機動（最大 |Δ*a*|=798 m→Δv≈0.42 m/s）在高訊噪比下可穩健觀測與反演。據此，我們主張低軌機動偵測門檻應由**固定絕對值改為高度相依之 σ 正規化（SNR 判定）**，以同時兼顧高軌靈敏度與低軌誤報控制。
