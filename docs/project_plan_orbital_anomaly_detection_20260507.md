...
### 1.2 現有基礎（截至 2026-05-07）

| 資產 | 狀態 |
|------|------|
| MEME ephemeris 下載 pipeline | ✅ 已自動化，日批次下載 284 顆 Starlink |
| TLE 歷史資料庫（DuckDB） | ✅ 4590 萬筆，39,009 顆衛星 |
| MEME-based 機動偵測 | ✅ `detect_maneuvers.py`，閾值法，J2 校正 |
| Ground Truth 標記集 | ✅ 3381 筆 transition labels（May 2–6，3天） |
| 軌道相位偵測模型 | ✅ XGBoost 4-class，CV Acc = 96.1% |
| 活動強度偵測模型 | ✅ XGBoost binary，CV AUC = 100.0% |

### 1.3 關鍵技術挑戰

1. **時間解析度差距**：TLE 每日 1–3 筆（~8h 積分），MEME 每 8h 一筆（瞬時快照）。個別推力在 TLE 中正負相消，需要統計方法重建
2. **微型機動偵測**：TLE 位置誤差約 100–300 m，直接從 TLE 偵測 10 m 量級機動需要特殊統計方法（CUSUM、SSA）
3. **標記資料稀少**：目前僅 3 天 MEME 資料；需要累積 + 合成資料擴充
4. **低誤報率要求**：Starlink 衛星「隨時都在機動」，需區分「正常操作」vs「異常事件」
...
```
Layer 1 — 閾值基準層（已完成）
  輸入：MEME 8h snapshots
  方法：J2-corrected Δa / Δi / ΔΩ 閾值
  精度：1 km（small）~ 10 km（large）
  用途：Ground Truth 標記，實時監控
...
- [ ] 稽核 TLE 資料庫雜訊特性
  - 計算真實 TLE vs MEME 殘差（按高度殼層分類）
  - 輸出：`data/noise_profile/tle_noise_characterization.csv`
- [ ] 擴充 MEME 下載覆蓋範圍
  - 確認每日批次正確執行（排程代理人已啟動）
  - 建立 MEME 完整性監控儀表板
- [ ] 建立 Benchmark 資料集 v0（當前 3381 筆標記）
...
- [ ] 強化 `detect_maneuvers.py`
  - 加入 3-sigma 異常過濾（去除 TLE fitting artifacts）
  - 改善 phasing 分類（延長觀察窗口至 14 天）
  - 輸出：per-transition 信賴分數（`confidence_score`）
- [ ] 建立 ISS 驗證集
...
- [ ] 重新執行 `detect_maneuvers.py`，更新標記集（目標 > 10,000 transitions）
- [ ] 交付 **Benchmark v1**

**月底交付物**：
- `generate_synthetic_tle.py`（合成資料生成器）
- `data/benchmark/benchmark_v1.csv`（真實 + 合成，各 1000+ 樣本）
- `data/noise_profile/`（TLE 雜訊特性報告）
...
- [ ] 重建 `build_training_dataset.py`（Phase B 特徵版本）
  ...
  - 延長 TLE 窗口至 30 天
  - 目標樣本數：~30,000 transitions（1個月 × 283衛星 × 3筆/天）
- [ ] 重新訓練 XGBoost（任務 A：Binary，任務 B：severity）
...
- [ ] 實作 `detect_burn_epoch.py`
  ...
  - **輸入特徵**：30 天 TLE context + Phase B 統計特徵
...
- [ ] 實作 `model_lstm_autoencoder.py`
  ...
  輸入：[sma_norm, i_norm, Ω_res_norm, e_norm] × 60 時步
...
- 技術報告：LSTM AE vs BOCPD vs CUSUM 比較
...
- [ ] 整合三層偵測架構為統一介面 `orbit_anomaly_detector.py`
  ...
  # returns: AnomalyReport with events, severity, confidence
  ```
...
| 幅度 | ΔV 範圍 | TLE 可偵測性 | MEME 可偵測性 | 目標 TPR |
|------|---------|-------------|--------------|---------|
| 微型 | < 0.1 m/s（Δa < 0.1 km） | ❌ 雜訊以下 | ✅ 勉強可見 | > 0.50 |
| 小型 | 0.1–2 m/s（Δa 0.1–5 km） | ⚠ 需統計方法 | ✅ 清晰可見 | > 0.70 |
| 中型 | 2–15 m/s（Δa 5–15 km） | ✅ 可偵測 | ✅ 清晰可見 | > 0.90 |
| 大型 | > 15 m/s（Δa > 15 km） | ✅ 明顯 | ✅ 極清晰 | > 0.98 |

### 5.3 比較基準

| 方法 | 說明 |
|------|------|
| **Baseline-Threshold** | 現有 `detect_maneuvers.py`（J2 校正 + 閾值） |
| **Baseline-CUSUM** | 統計變化點偵測（無 AI） |
...
| MEME ephemeris | <10 m | 0.02 km（20 m） | 8h 解析度 |
...
1. **MEME 直接偵測**：MEME 精度 < 10 m，連續快照能量差可偵測 |ΔV| > 0.05 m/s
...
| 里程碑 | 日期 | 交付物 |
|--------|------|--------|
| **M1** Data Foundation | 2026-06-06 | 合成資料生成器、Benchmark v1、雜訊特性報告 |
| **M2** Statistical Layer | 2026-07-06 | CUSUM+BOCPD+SSA 偵測器、ML v2（1個月GT） |
| **M3** Burn-Epoch Detection | 2026-08-06 | 8h 精度機動時刻偵測、LSTM AE、Benchmark v2 |
| **M4** Constellation + DL | 2026-09-06 | 星系級異常偵測、Patch Transformer、微型機動分析 |
| **M5** Integration & Validation | 2026-10-07 | 統一介面、Benchmark v3、最終技術報告 |
...
|------|------|
| 語言 | Python 3.11+ |
| 軌道力學 | `numpy`、`scipy`（J2/J4 攝動）、`astropy`（座標轉換） |
| 資料庫 | DuckDB（TLE + MEME 標記集） |
| ML 框架 | XGBoost、scikit-learn、PyTorch（LSTM / Transformer） |
| 統計方法 | `ruptures`（BOCPD/PELT）、`scipy.signal`（SSA/LSP） |
| 可視化 | Plotly、Matplotlib、Streamlit |
...
| 風險 | 可能性 | 影響 | 因應措施 |
|------|--------|------|----------|
| Starlink MEME API 停止服務 | 低 | 高 | 合成資料補充；切換 ISS + 其他衛星 |
| 3 個月標記資料仍不足 | 中 | 中 | 強化合成資料生成；Transfer Learning |
| TLE 精度限制微型機動 | 高 | 中 | 調整偵測目標下限至 200 m；MEME only 路徑 |
