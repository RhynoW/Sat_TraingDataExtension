#!/usr/bin/env python3
"""
maneuver_app_2026August.py — 緊湊版機動偵測儀表板（2026-08 定版，前身 maneuver_app_july.py）
=====================================================
配合最新 ML（models_meme_forecast）與統計偵測（CUSUM/BOCPD/SSA）模型的重寫版。
用法：  streamlit run maneuver_app_2026August.py
...
st.set_page_config(page_title="機動偵測儀表板 July", page_icon="🛰️", layout="wide")
...
def plot_elements_and_deltas(df: pd.DataFrame, tr: pd.DataFrame, combined: np.ndarray,
    ...
    """① a/i/e/RAAN 連續時序（左）+ Δ 差值（右），共 4 列。
    ...
    fig = make_subplots(
        rows=4, cols=2, shared_xaxes=True, horizontal_spacing=0.08, vertical_spacing=0.04,
        subplot_titles=(a_title, "Δa (km)", "傾角 i (deg)", "Δi (deg)",
                        "離心率 e", "Δe", "升交點赤經 RAAN (deg)", "ΔRAAN 殘差 (deg)"))
    ...
    fig.update_layout(height=760, margin=dict(l=40, r=20, t=40, b=30),
...
RAG_DEFAULT_URL = "http://127.0.0.1:8000"
