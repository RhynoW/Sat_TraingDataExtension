#!/usr/bin/env python3
"""
maneuver_app_2026SOctober.py — 機動偵測儀表板（2026-10 版，前身 maneuver_app_2026September.py）
=====================================================
配合最新 ML（models_meme_forecast）與統計偵測（CUSUM/BOCPD/SSA）模型的重寫版。
用法（本機）：  streamlit run maneuver_app_2026SOctober.py

【與 September 版差異】
  * compute_ml_detection／compute_fusion_detection／build_tle_maneuver_narrative／
    build_ml_maneuver_narrative 四個函式改為呼叫 maneuver_report_builder.py
    （新增的 Streamlit 無關共用模組），供獨立部署的 report_api.py（PDF 報表 API）
    共用同一份邏輯，避免兩邊各存一份、日後改一邊忘了改另一邊。此檔內行為不變。
...
def compute_ml_detection(norad: int, d0, d1):
    ...
    邏輯已搬到 maneuver_report_builder.py（供 report_api.py 共用），此處僅加 Streamlit 快取。"""
    return rb.compute_ml_detection(norad, d0, d1)
...
def compute_fusion_detection(norad: int, d0, d1):
    ...
    邏輯已搬到 maneuver_report_builder.py（供 report_api.py 共用），此處僅加 Streamlit 快取。"""
    return rb.compute_fusion_detection(norad, d0, d1)
...
def build_tle_maneuver_narrative(satellite_id, alt_km_avg, start_date: str,
    ...
    邏輯已搬到 maneuver_report_builder.py（供 report_api.py 共用），此處僅轉呼叫。"""
    return rb.build_tle_maneuver_narrative(satellite_id, alt_km_avg, start_date, end_date, event_df)
...
def build_ml_maneuver_narrative(satellite_id, p_maneuver: float, lgbm_feat: dict,
    ...
    邏輯已搬到 maneuver_report_builder.py（供 report_api.py 共用），此處僅轉呼叫。"""
    return rb.build_ml_maneuver_narrative(satellite_id, p_maneuver, lgbm_feat, start_date, end_date, alert)
...
def render_storymap_case14():
    ...
    if _cr is not None and len(_cr):
        ...
        if len(_fs7_df):
            ...
            st.warning(T3(
                "**誠實判讀（2026-09-14 訂正）**：四法在福衛七號上之 F1 全數落於 "
                "0.07-0.24，明顯低於同批方法在其餘 23 星標竿上的表現（0.31-0.46 級，"
                "已重新核算）——**這正是上方預期的物理解析度限制被證實**。**平均值**"
                "排名：Polynomial（0.239）＞LOSO L3融合（0.216）＞LOWESS（0.152）＞"
                "iter2 k=8（0.071）。**但逐星細看，L3 融合 vs Polynomial 並非單方面"
                "落後，而是接近五五波**：6 顆衛星中 L3 融合在 3 顆（FORMOSAT7-2/4/5）"
                "F1 較高、Polynomial 在另 3 顆（FORMOSAT7-1/3/6）較高，平均值之所以"
                "偏向 Polynomial，主要是 FORMOSAT7-3（0.500 vs 0.385）與 "
                "FORMOSAT7-6（0.231 vs 0.197）兩顆差距較大所致，**「L3融合仍未超越"
                "Polynomial」若不加說明容易被誤讀為單方面落後，應訂正為「兩者互有"
                "勝負，平均值因少數衛星拉開差距」**。可確定的結論仍是：(1) L3 融合"
                "遠優於本專案簡單規則式 iter2（0.216 vs 0.071），方向與 23 星標竿"
                "一致；(2) 四法在此皆明顯受限於福衛七號機動量級（數十公尺）已被 "
                "TLE 雜訊淹沒之物理解析度限制，如實記錄而非隱藏。",
                "**誠実な判読（2026-09-14訂正）**：4手法のフォルモサット7号でのF1は"
                "全て0.07-0.24の範囲にあり、同じ手法群の他の23機ベンチマークでの成績"
                "（0.31-0.46級、再計算済み）よりも明らかに低い——**これはまさに上記で"
                "予想された物理的解像度の限界が実証されたものである**。**平均値**の"
                "ランキング：Polynomial（0.239）＞LOSO L3融合（0.216）＞LOWESS"
                "（0.152）＞iter2 k=8（0.071）。**しかし衛星ごとに詳しく見ると、L3融合"
                "とPolynomialは一方的に劣っているのではなく、ほぼ互角である**：6機の"
                "うちL3融合が3機（FORMOSAT7-2/4/5）でF1が高く、Polynomialが残りの"
                "3機（FORMOSAT7-1/3/6）で高い。平均値がPolynomialに偏っているのは"
                "主にFORMOSAT7-3（0.500 vs 0.385）とFORMOSAT7-6（0.231 vs 0.197）の"
                "2機で差が大きいためであり、**「L3融合はPolynomialを上回れなかった」"
                "という表現は説明なしでは一方的な敗北と誤解されやすく、「両者は勝敗"
                "半々で、少数の衛星が平均値の差を広げている」と訂正すべきである**。"
                "確実な結論は依然として：(1) L3融合は本プロジェクトの単純なルール"
                "ベースのiter2を大きく上回っており（0.216 vs 0.071）、23機ベンチマーク"
                "と方向性が一致する；(2) 4手法ともここではフォルモサット7号の機動"
                "規模（数十メートル）がTLEノイズに埋もれているという物理的解像度の"
                "限界に明らかに制約されており、隠さず誠実に記録する。",
                "**Honest interpretation (corrected 2026-09-14)**: all four methods' F1 on "
                "FORMOSAT-7 fall in the 0.07-0.24 range, clearly below the same methods' "
                "performance on the rest of the 23-satellite benchmark (0.31-0.46, "
                "recalculated) — **exactly confirming the physical resolution limit "
                "anticipated above**. **Average-value** ranking: Polynomial (0.239) > LOSO "
                "L3 fusion (0.216) > LOWESS (0.152) > iter2 k=8 (0.071). **But looking "
                "satellite by satellite, L3 fusion vs. Polynomial is not one-sided at all — "
                "it's close to a coin flip**: of the 6 satellites, L3 fusion has the higher "
                "F1 on 3 (FORMOSAT7-2/4/5) and Polynomial on the other 3 (FORMOSAT7-1/3/6); "
                "the average leans toward Polynomial mainly because of two satellites with a "
                "larger gap — FORMOSAT7-3 (0.500 vs. 0.385) and FORMOSAT7-6 (0.231 vs. "
                "0.197). **Saying \"L3 fusion still fails to beat Polynomial\" without this "
                "context is easily misread as a one-sided loss; it should be corrected to "
                "\"the two trade wins, and a couple of satellites widen the average gap.\"** "
                "What remains solid: (1) L3 fusion substantially outperforms this project's "
                "simple rule-based iter2 (0.216 vs 0.071), consistent in direction with the "
                "23-satellite benchmark; (2) all four methods here are clearly constrained by "
                "FORMOSAT-7's maneuver scale (tens of meters) being buried in TLE noise — a "
                "physical resolution limit, reported honestly rather than hidden.",
            ))
            st.caption(T3(
