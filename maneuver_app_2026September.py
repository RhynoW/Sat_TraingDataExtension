...
def render_storymap_case3():
    ...
    # ── 情境一：抬軌（真實資料）──────────────────────────────────────────────
    st.header("情境一：抬軌階段——1～2 天的典型 TLE 頻率就已經是壓倒性證據")
    if "raise_df" in data and not data["raise_df"].empty:
        ...
        if rate is not None:
            ...
            st.markdown(
                ...
                f"**單一天的變化量對應 SNR≈{snr_1day:.0f}σ**——遠超任何合理判定門檻（通常 2–3σ 即視為明確訊號）。"
    ...
    # ── 情境二：站位保持（真實資料，量測雜訊底）──────────────────────────────
    st.header("情境二：站位保持——真實雜訊底長什麼樣，抓到一次真實的小型修正")
    ...
    st.header("情境三：如果同樣的位移拆成好幾天執行——校準過的模擬實驗")
    st.markdown(
    ...
    st.success(
        ...
        "現有的**逐點線上偵測器**（CUSUM／BOCPD／3σ-MAD）之 per-step SNR 就會跌破可靠判定門檻——"
...
def render_storymap_case4():
    ...
    st.header("一、三個真值來源——這 23 顆星的真值不是同一個來源")
    st.markdown(
    ...
    st.header("二、14 顆開發 vs 9 顆 hold-out 的分組與來源歸屬")
    st.markdown(
        "| 分組 | 衛星 | 來源 |\n"
        "|---|---|---|\n"
    ...
    st.header("四、真實案例：Jason-3 的一次「升軌又降回來」站位保持")
...
def render_storymap_case5():
    ...
    st.markdown(
        "**問題背景**：低軌衛星的半長軸每天都在變小——這是大氣阻力造成的自然衰減，太陽活動越強（F10.7 越高）"
...
def render_storymap_case7():
    ...
    st.header("事件一：TJS-10 抵近 TJS-3（GEO，2024 年春）")
    tjs = data.get("tjs")
    if tjs:
        s = tjs["summary"]
    ...
    st.header("事件二：ISS 對接 Cygnus NG-24 貨運飛船（LEO，2026 年 4 月）")
    cyg = data.get("cygnus")
    if cyg:
        s = cyg["summary"]
...
def render_storymap_case9():
    ...
    st.markdown("---")
    st.success(
        "**判讀**：AUC 0.98、大型機動召回率 100%（81 個大型事件全中，Wilson 95% CI [0.955, 1.000]）——"
...
def render_storymap_case13():
    ...
    st.warning(
        ...
        "真正能定位點火的方法是**檔案內部**逐點用 vis-viva 方程式算瞬時半長軸——推力弧會在單一檔案內直接顯現。"
