        ...
        if pr["status"] != "ok":
            ...
            lines.append(
                ...
                + "。**此指標僅供參考，不併入上述 L1/L2/L3 判定或機動偵測落點清單**："
                  "獨立驗證顯示其準確度遠低於本報表主要偵測方法——Starlink 星系"
                  "（23 星標竿 macro F1≈0.10；207 星擴大樣本 macro F1≈0.21，"
                  "兩者真值來源不同、非嚴格可比）之外，跨軌道類型無真值廣泛掃描"
                  "（LEO/MEO/GEO/HEO 各 125 顆）顯示 LEO/MEO/HEO 候選觸發率"
                  "高達 76–84%（門檻未針對非 Starlink 族群校準，接近雜訊水準），"
                  "完整驗證工作尚未完成，結果應視為候選假說而非已驗證結論"
                  "（詳見 docs/相位殘差通道_新增偵測管道實作與驗證_20260913.md）。")
            if r.get("orbit_class") in ("GEO", "GEO+", "HEO"):
                lines.append(
                    "**額外警示**：本衛星軌道分類為 GEO／HEO，無真值廣泛掃描"
                    "顯示相位殘差訊號在此類軌道下可能存在未診斷之數值穩定性"
                    "問題（GEO 樣本曾觀測到物理上不合理之殘差量級），上述"
                    "候選次數與殘差量級之參考價值更為有限，建議直接忽略本欄。")
    return "\n".join(lines)
