...
# =================== Streamlit App ===================
def main():
    st.title("Starlink Shell TLE 分析面板")
    ...
    shell_name = shell_cfg.get("shell_name", "Starlink Shell")
    ...
    # 理論相鄰 plane RAAN 間距（度）
    theoretical_spacing_deg = round(360.0 / num_planes, 4)
    ...
    st.session_state["shell_name"] = shell_name
    st.session_state["theoretical_spacing_deg"] = theoretical_spacing_deg
    ...
    with tab_sma:
        ...
        if df_all.empty:
            ...
            ax.set_title(f"{shell_name} Shell 重構軌跡")
    ...
    with tab_raan:
        ...
        if df_raan_all.empty:
            ...
            if df_p.empty:
                ...
                ax.set_ylim(0, 360)
                ax.set_title(f"{shell_name} {plane_sel} 每日平均 RAAN")
    ...
    with tab_heat:
        ...
        if df_raan_all.empty:
            ...
            ax.set_title(f"{shell_name} 每日平均 RAAN 群聚圖")
    ...
    with tab_delta:
        ...
        shell_name_disp = st.session_state.get("shell_name", "Starlink Shell")
        spacing = st.session_state.get("theoretical_spacing_deg", 5.0)
        ...
        if df_delta_adj.empty or not balanced_plane_ids:
            ...
            if df_pair.empty:
                ...
                ax.axhline(spacing,  color="r", ls="--", lw=1,
                           label=f"理論間距 +{spacing:.2f}°")
                ax.axhline(-spacing, color="r", ls="--", lw=1,
                           label=f"理論間距 -{spacing:.2f}°")
                ax.set_xlabel("日期 (UTC)")
                ...
                ax.set_title(
                    f"{shell_name_disp}: {plane_i} (NORAD {norad_i}) vs {plane_j} (NORAD {norad_j})"
                )
    ...
    if st.sidebar.button("清除 ΔRAAN cache"):
        for key in ["df_delta_adj", "plane_ids", "num_planes",
                    "shell_name", "theoretical_spacing_deg", "balanced_plane_ids"]:
